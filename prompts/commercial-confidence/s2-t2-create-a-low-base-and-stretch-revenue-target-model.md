# Create a low, base and stretch revenue target model
...
**Task goal:** A target model that separates evidence from pressure.

**Success test:** One target can be defended without relying on emotion or pressure.
...
```text
Use ChatGPT Project. If this needs document tone, long writing, or Microsoft 365 context, use DeepSeek.

I am working on this roadmap task: Create a low, base and stretch revenue target model.

Why it exists from my questionnaire answers:
You said revenue targets often feel unsupported, forecasting confidence is almost zero, and leadership feedback can ignore pricing shock and missing data.
...
Problem: Revenue targets feel political.
Cause: Targets are not consistently built from data, assumptions, pipeline coverage and risk.
...
Task: Create a low, base and stretch revenue target model
Required output: A target model that separates evidence from pressure.
Minimum version: Create low, base and stretch numbers with one assumption each.
Full version: A target model that separates evidence from pressure.
...
One target can be defended without relying on emotion or pressure.
