...
**Task goal:** One tiny artifact, app or automation based on a proven workflow.

**Success test:** One small build shipped or consciously killed.
...
I am working on this roadmap task: Build one small tool only if the workflow earned it.

Why it exists from my questionnaire answers:
You said you love tools but fear building something useless. This task only allows a build if the workflow has earned it.
...
Problem: Building tools too early creates waste.
Cause: A workflow must earn a small build.
...
Task: Build one small tool only if the workflow earned it
Required output: One tiny artifact, app or automation based on a proven workflow.
Minimum version: Complete the smallest version that creates a real-world action within 48 hours.
Full version: One tiny artifact, app or automation based on a proven workflow.
